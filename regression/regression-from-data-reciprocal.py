...
def fn_error_deriv(a, j, x, y):
	e = 0
	for i in range(len(x)):
		e += (2.0 * x[i]**j) * ((y[i] / (poly(a, x[i])**2)) - (1.0 / (poly(a, x[i])**3)))
	return e

def fn_error_deriv_2(a, j, x, y):
	e = 0
	for i in range(len(x)):
		e += (2.0 * x[i]**(2 * j)) * ((3.0 / (poly(a, x[i])**4)) - (2 * y[i] / (poly(a, x[i])**3)))
	return e
...
def draw_graph(config, a):
	xrange = np.linspace(0.01, 1.01, 100)
	yvals = [fn(a, x) for x in xrange]

	ax = config.add_subplot()
	ax.set_title('$\\frac{{1}}{{ {} }}$'.format(poly_string(a)), fontsize=config.fontsize)
...
def draw_graphs(config, a, atest, polynomial, x, y):
	...
	yrecipvals = [1.0 / yval for yval in np.poly1d(polynomial)(xrange)]
	plt.plot(xrange, yrecipvals, color='orange')
...
def linear_regression(degree, x, y):
	...
	polynomial = np.polyfit(x, yrecip, degree, w = w)
	return polynomial
...
def print_syntax():
	...
	print()
	print('\tPerforms a reciprocal polynomial regression using data from the file provided.')
...
print('Recipr : {}'.format(atest))

print()
print('# Function expressions')
print()
print('Linear f(x) = 1 / {{{}}}'.format(poly_string(apoly)))
print('Recipr f(x) = 1 / {{{}}}'.format(poly_string(atest)))
...
print('Recipr E = {}'.format(fn_error(atest, x, y)))

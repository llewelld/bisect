...
For each commit triple $(A, B, C)$ we normalise $B$ to get a value $B'$ which lie between 0 and 1 thus:
$$
B' = \frac{B - A}{C - A} .
$$

Based on our assumptions, the mean $\mu$ of all the $B'$ values will be 0.5 and the standard deviation $\sigma$ will be $1 / \sqrt{12} \approx 0.289$. The Central Limit Theorem tells us that the mean of our sampling will be $\mu$ and the standard deviation $1 / \sqrt{12 n}$ where $n$ is our sample size (see Goucher and Riley \cite{goucher2009}). If our sample is indeed from a uniform distribution, then the mean will fall into the range
